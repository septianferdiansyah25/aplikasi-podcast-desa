...
def generate_multiselect_pdf(podcasts):
    """
    Menghasilkan PDF Laporan untuk beberapa podcast yang dipilih.
    """
    ...
    # Header
    pdf.set_font("Arial", 'B', 16)
    pdf.cell(0, 15, "LAPORAN DATA PODCAST DESA", ln=True, align='C')
    pdf.ln(5)
    ...
    # Column Widths (A4 Landscape total: 277mm)
    col_widths = [60, 35, 45, 30, 35, 72]
    headers = ["Judul Podcast", "Kategori", "Narasumber", "Tanggal", "Status", "Deskripsi Singkat"]
    ...
    for p in podcasts:
        ...
        status = clean_text_for_pdf(p.status.upper())
        desc = clean_text_for_pdf(p.deskripsi[:100] + "..." if len(p.deskripsi) > 100 else p.deskripsi)
        
        pdf.cell(60, 10, judul[:35] + ("..." if len(judul) > 35 else ""), border=1)
        pdf.cell(35, 10, kategori, border=1, align='C')
        pdf.cell(45, 10, narsum[:25], border=1)
        pdf.cell(30, 10, tanggal, border=1, align='C')
        pdf.cell(35, 10, status, border=1, align='C')
        pdf.cell(72, 10, desc[:45], border=1)
        pdf.ln()
        
    filename = "laporan_podcast_terpilih.pdf"
    filepath = os.path.join('static', 'pdf', filename)
    pdf.output(filepath)
    return filename
    return filename
